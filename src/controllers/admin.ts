 ...
 */
export const allChats = TryCatch(async (req: Request, res: Response) => {
  // Fetch all chats with populated members and creator
  const chats = await Chat.find({})
    .populate<{ members: IUser[] }>("members", "name avatar")
    .populate<{ creator: IUser }>("creator", "name avatar");
 ...
 */
export const allMessages = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    // Fetch all messages with populated sender and chat
    const messages = await Message.find({})
      .populate<{ sender: IUser }>("sender", "name avatar")
