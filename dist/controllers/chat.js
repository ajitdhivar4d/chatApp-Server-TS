...
export const leaveGroup = TryCatch(async (req, res, next) => {
    ...
    const userId = typeof req.user === "string" ? new Types.ObjectId(req.user) : req.user;
    const remainingMembers = chat.members.filter((member) => member.toString() !== userId.toString());
...
export const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    const bufferFiles = await Promise.all(files.map(async (file) => await fileToBuffer(file)));
...
export const getMessages = TryCatch(async (req, res, next) => {
    ...
    const userId = req.user; // Type assertion to ObjectId
